...
-  주문 탭 => 주석처리 되어있던 Delivery 메뉴 삭제
   (배송 정보를 따로 구분하는 것이 아닌 주문 정보 안에 추가)

---

> 2022.03.30

-  주문내역 process => 결제완료 & 처리완료와 default 구분

---

> 2022.03.29

-  일부 컴포넌트 React.memo 사용으로 불필요한 리렌더링 방지
-  '통계' 메뉴 삭제
...
-  eslint 에러 해결
-  주문내역 비동기요청 처리중 로딩화면 추가
-  주문내역 페이지 변동 시 화면에 반영되지 않는 오류 수정
   _useEffect에 page를 참조하여 리스트를 띄우고 싶었으나 '적용하기' 버튼을 누르기 전인 초기 렌더링 시에는 리스트를 띄우지 않아야 함. => useDidMountEffect 커스텀 훅을 사용해서 원하는 방식으로 작동시킴_
   _1쪽이 아닌 다른 쪽을 보던 중 필터 변경 후 "적용하기" 클릭 시 page를 다시 1로 초기화하는 로직 추가_
